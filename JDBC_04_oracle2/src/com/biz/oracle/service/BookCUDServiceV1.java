...
public class BookCUDServiceV1 {
	...
	public void updateBook() {
		...
		System.out.println("=========================================");
		System.out.print("수정할 도서 코드(-Q : quit) >> ");
		String strCode = scanner.nextLine();
		
		if(strCode.equals("-Q"));  // 입력을 끝내기
		// 키보드로 입력받은 코드에 해당하는 도서정보를 가져오고
		// 그 결과를 bookDTO에 전달
		BookDTO bookDTO = bookDao.findById(strCode);
		 ...
		 * 		기존의 Data를 그대로 유지하고 새로운 데이터를 INSERT 한다.
		 */
		// 각 항목별로 값을 수정
		...
		// 새로운 도서명을 입력했을 때는 
		// bookDTO의 도서명 변수 필드를 새로운 도서명으로 대치하고
		// 그냥 Enter만 입력했을때는 변경하지 않는다.
		// .trim() 실수로 공백(space)가 입력될때를 대비하여
		// 입력된 도서명의 앞뒤 공백을 제거하고 검사
		// strName.trim().length() : 객체의 chaining방법 
		...
		if(strWriter.trim().length() > 0) {
			bookDTO.setB_comp(strWriter.trim());
		}
		...
		// 만약 값으 입력하지 않고 Enter를 누르면
		// NUMBER FOTMAT EXCEPTION이 발생하는 것을 역 이용하여
		...
		// 여기 도착했을때 bookDTO에 담긴 값은
		// 처음에 table에서 읽은 값이 저장되어 있거나
		// 중간에 키보드로 입력한 값으로 변경되었거나함
		// 이값을 BookDaoImp의 update로 보내서 update실행
